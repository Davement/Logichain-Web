          ...
          if (Array.isArray(config.providers)) {
            config.providers.forEach(cProvider => {
              if (cProvider instanceof Object) {
                let index = moduleDef.providers!.findIndex((mProvider: any) => mProvider && mProvider.provide ? mProvider.provide === cProvider.provide : (<Function>mProvider).name === (<Function>cProvider).name);
                if (index === -1) {
